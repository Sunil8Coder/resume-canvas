...
export const aiService = {
  async enhanceText(
    prompt: string,
    systemPrompt = "You are an expert resume writer. Only return the improved text, no explanations.",
  ...
  async improveSummary(summary: string): Promise<{ data?: string; error?: string }> {
    return this.enhanceText(
      `Improve this professional summary for a resume. Keep it concise (3-4 sentences). Only return the improved summary text, nothing else:\n\n${summary}`,
    );
  },
